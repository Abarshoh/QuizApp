...
class MainActivity : AppCompatActivity() {
    ...
    private fun getTests():ArrayList<Question>{
        ...
        val library_size = library.size
        if (testCount == library_size){
            for (i in library){
        ...
        for (i in 1..testCount){
            var randomNum = (0 until library_size).random()
            while (randoms.contains(randomNum)){
                randomNum = (0 until library_size).random()
            }
            randoms.add(randomNum)
            val a = library[randomNum]
            val vars = a.vars
            vars.shuffle()
